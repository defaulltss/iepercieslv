...
if (isset($_POST['signup-submit'])) {
    ...
    if (empty($username) || empty($usersurname) || empty($email) || empty($password) || empty($passwordRepeat)) {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
            ...
            if ($resultCheck > 0) {
                ...
                $sql = "INSERT INTO users (users_firstname, users_lastname, users_uid, users_pwd) VALUES (?, ?, ?, ?)";
                $stmt = mysqli_stmt_init($conn);
                if (!mysqli_stmt_prepare($stmt, $sql)) {
                    ...
                    mysqli_stmt_bind_param($stmt, "ssss", $username, $usersurname ,$email, $hashedPwd);
                    mysqli_stmt_execute($stmt);
